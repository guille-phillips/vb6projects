...
Private oStatement As ISaffronObject
...
Private Sub Form_Load()
    ...
    sDefinition = Space$(FileLen(App.Path & "\calculator.saf"))
    Open App.Path & "\calculator.saf" For Binary As #1
    Get #1, , sDefinition
    Close #1
    ...
    Set oStatement = Rules("level0")
    
'    Dim oTree As SaffronTree
'    Dim oRule As ISaffronObject
'
'    Set oTree = New SaffronTree
'    Set oRule = Rules("level0")
'
'    SaffronStream.Text = "a==7"
'
'    If oRule.Parse(oTree) Then
'        Stop
'    End If
'
'    End
    ...
    Select Case KeyAscii
        Case 13
            ...
            If oStatement.Parse(oResult) Then
                If SaffronStream.Position = Len(sCommand) + 1 Then
...
Private Function EvaluateLevel0(ByVal oExpression As SaffronTree, Optional oParameters As Dictionary) As String
...
Private Function EvaluateLevel1(ByVal oExpression As SaffronTree, Optional oParameters As Dictionary) As String
    On Error GoTo ExitPoint
    Dim sErrorSource As String
    ...
    Select Case oExpression.Index
        Case 1 ' Keyword
            Select Case LCase$(oExpression.Text)
                Case "list"
                    ListDefinitions
                Case "help"
                    InsertText vbCrLf & "SIN, COS, TAN, EXP, LOG, DEG, RAD, INT, FRAC, SQR, NOT, ATN, ACS, ASN, GAM, COT, CSC, SEC, DM, DMS, HM, HMS, LEN, BASE, LOG, FIX, IF, AND, OR, XOR, MOD, LOOP, SHOW" & vbCrLf & "function(x):=func(x)" & vbCrLf & "loop/show+*(a,b) x:x"
            End Select
        ...
        Case 3 ' Function Assigment
            ...
            Dim oLevel0 As ISaffronObject
            Dim oResult As New SaffronTree
            
            Set oLevel0 = Rules("level0")
            sExpression = oExpression(1)(3).Text
            SaffronStream.Text = sExpression
        
            If Not oLevel0.Parse(oResult) Then
                Err.Raise -1
                Exit Function
            ElseIf SaffronStream.Position <> Len(sExpression) + 1 Then
                Err.Raise -1
                Exit Function
            End If
            
            newvar.Expression = sExpression
...
Private Function EvaluateLevel2(ByVal oExpression As SaffronTree, Optional oParameters As Dictionary) As String
    Dim runningvalue As String
    Dim vPart As Variant
    Dim lPartIndex As Long
    Dim tempvalue As String
...
Private Function EvaluateLevel3(ByVal oExpression As SaffronTree, Optional oParameters As Dictionary) As String
    Dim runningvalue As String
    Dim vPart As Variant
    Dim lPartIndex As Long
    Dim tempvalue As String
...
Private Function EvaluateLevel4(ByVal oExpression As SaffronTree, Optional oParameters As Dictionary) As String
    Dim runningvalue As String
    Dim vPart As Variant
    Dim lPartIndex As Long
...
Private Function EvaluateLevel5(ByVal oExpression As SaffronTree, Optional oParameters As Dictionary) As String
    Dim runningvalue As String
    Dim vPart As Variant
    Dim lPartIndex As Long
...
Private Function EvaluateLevel6(ByVal oExpression As SaffronTree, Optional oParameters As Dictionary) As String
    Dim runningvalue As String
    Dim vPart As Variant
    Dim lPartIndex As Long
...
Private Function EvaluateLevel7(ByVal oExpression As SaffronTree, Optional oParameters As Dictionary) As String
    Dim runningvalue As String
    ...
    Dim oSubExpression As SaffronTree
    ...
    Select Case oExpression.Index
        ...
        Case ftTime
            If oExpression(1).Index = 1 Then
                hour = oExpression(1)(1)(1).Text
                minute = oExpression(1)(1)(3).Text
                second = oExpression(1)(1)(5).Text
                fraction = "0" & oExpression(1)(1)(6).Text
                runningvalue = hour + (minute + (second + fraction) / 60) / 60
            Else
                hour = oExpression(1)(1)(1).Text
                minute = oExpression(1)(1)(3).Text
                fraction = "0" & oExpression(1)(1)(4).Text
                runningvalue = hour + (minute + fraction) / 60
            End If
        Case ftRadixNumber
            ...
            Select Case UCase$(oExpression(1)(1).Text)
                ...
                Case "DM"
                    ...
                    If runningvalue < 0 Then
                        runningvalue = runningvalue - 360 * Int(runningvalue / 360)
                    End If
                    degree = Int(runningvalue)
                    minute = (runningvalue - degree) * 60
                    runningvalue = Format(degree, "000") & ":" & Format(minute, "0#.###############")
                    If Right$(runningvalue, 1) = "." Then
                        runningvalue = Left$(runningvalue, Len(runningvalue) - 1)
                    End If
                Case "DMS"
                    runningvalue = EvaluateLevel0(oSubExpression, oParameters)
                    If runningvalue < 0 Then
                    ...
                    End If
                    degree = Int(runningvalue)
                    minute = Int((runningvalue - degree) * 60)
                    second = ((runningvalue - degree) * 60 - minute) * 60
                    runningvalue = Format(degree, "000") & ":" & Format(minute, "00") & ":" & Format(second, "0#.###############")
                    If Right$(runningvalue, 1) = "." Then
                        runningvalue = Left$(runningvalue, Len(runningvalue) - 1)
                    End If
                Case "HM"
                    runningvalue = EvaluateLevel0(oSubExpression, oParameters)
                    If runningvalue < 0 Then
                        runningvalue = runningvalue - 24 * Int(runningvalue / 24)
                    End If
                    hour = Int(runningvalue)
                    minute = (runningvalue - hour) * 60
                    runningvalue = Format(hour, "00") & ":" & Format(minute, "0#.###############")
                    If Right$(runningvalue, 2) = "." Then
                        runningvalue = Left$(runningvalue, Len(runningvalue) - 1)
                    End If
                Case "HMS"
                    runningvalue = EvaluateLevel0(oSubExpression, oParameters)
                    ...
                    End If
                    hour = Int(runningvalue)
                    minute = Int((runningvalue - hour) * 60)
                    second = ((runningvalue - hour) * 60 - minute) * 60
                    runningvalue = Format(hour, "00") & ":" & Format(minute, "00") & ":" & Format(second, "0#.###############")
                    If Right$(runningvalue, 1) = "." Then
                        runningvalue = Left$(runningvalue, Len(runningvalue) - 1)
                    End If
                Case "LEN"
                ...
                Case Else
                    ...
                    End If
                    If Left$(UCase$(oExpression(1)(1).Text), 4) = "BASE" Then
...
'Logarithm to base N LogN(X) = Log(X) / Log(N)
        ...
        Case ftFunctionVariableCall
            ...
            Dim oParameterValue As SaffronTree
            ...
            If oVariables.Exists(sVariableName) Then
                ...
                Else
                    Dim subdecode As ISaffronObject
                    Dim othisResult As New SaffronTree
                    Dim savetext As String
                    Dim savepos As String

                    savetext = SaffronStream.Text
                    savepos = SaffronStream.Position

                    Set subdecode = Rules("level0")

                    SaffronStream.Text = thevar.Expression
                    subdecode.Parse othisResult
                    runningvalue = EvaluateLevel0(othisResult, oVariables.Item(sVariableName).Parameters)
                    SaffronStream.Text = savetext
                    SaffronStream.Position = savepos
                End If
            Else
                sErrorSource = "VARIABLE"
                Err.Raise -3
...
Private Function ReadDefinitions()
    ...
    While Not oTS.AtEndOfStream
        ...
        If oStatement.Parse(oResult) Then
            If Position = Len(sCommand) + 1 Then
