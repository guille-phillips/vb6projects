...
function PopUp(Type, Country, Text, WindowName, Width) {
  ...
  if (Type == "EXTERNAL") {
    ...
    if (Doc&&Doc.innerHTML) {
      ...
      if (Type) {
        if (Type != 'TEXT') {
	      Contents = '<TABLE CELLSPACING=0 CELLPADDING=1 BORDER=0 WIDTH='+Width+' BGCOLOR="#D3D3D3"><TR><TD COLSPAN=2><B>'+Country+'</B></TD><TD ALIGN="RIGHT"><A HREF="" onClick="PopUp(0, \''+WindowName+'\');return false;"><IMG HSPACE=2 WIDTH=16 HEIGHT=14 SRC="/ipdl/include/close.gif" BORDER=0></A></TD></TR>'+eval(Type+"CommonMessage")+Text+'</TABLE>';
        }
        else {
	   	  if ((Country != "BODY") && (Country != 'NORMAL')) {
			  Contents = '<TABLE CELLSPACING=0 CELLPADDING=1 BORDER=0 WIDTH='+Width+' BGCOLOR="#D3D3D3"><TR><TD ALIGN="RIGHT"><A HREF="" onClick="PopUp(0, \''+WindowName+'\');return false;"><IMG HSPACE=2 WIDTH=16 HEIGHT=14 SRC="/ipdl/include/close.gif" BORDER=0></A></TD></TR><TR><TD>'+Text+'</TD></TR></TABLE>';
		  }
		  else {
			  Contents = Text;
		  }
	...
	self.popped = false;
	  ...
	  if (Type) {
        if (Type != 'TEXT') {
	      Doc.write('<TABLE CELLSPACING=0 CELLPADDING=3 BORDER=0 WIDTH='+Width+' BGCOLOR="#D3D3D3"><TR><TD COLSPAN=2 NOWRAP>'+Country+'</TD><TD ALIGN="RIGHT"><A HREF="" onClick="PopUp(0);return false;"><IMG SRC="/ipdl/include/close.gif" WIDTH=16 HEIGHT=14 BORDER=0></A></TD></TR>');
...
function fieldcodes() {
	testwin = open('about:blank','FieldCodes','height=400,width=595,scrollbars=yes,toolbars=no,resizable=yes');
